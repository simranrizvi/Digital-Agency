...
export default function Navbar() {
  ...
  useEffect(() => {
    if (pathname === "/") {
      ...
      if (!heroSection) {
        setShowNavbar(true); // agar hero section na mile to navbar dikhayen
        return;
      }
      ...
      const observer = new IntersectionObserver(
        ([entry]) => {
          if (entry.isIntersecting) {
            setShowNavbar(false); // hero section visible hai, navbar chhupao
          } else {
            setShowNavbar(true); // hero section chhupa hai, navbar dikhao
          }
        },
        { threshold: 0.1 }
      );
      ...
      // Baaki pages par hamesha navbar dikhao
      setShowNavbar(true);
    }
  }, [pathname]);
